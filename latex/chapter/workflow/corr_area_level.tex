...
Here, the actual correlation matrix is depicted, \textit{not} the precision matrix $\Omega$.\footnote{$\Omega^{-1}$ is a covariance matrix, which is rescaled with the \code{cov2cor} function in \code{R}. The scaling parameter $\sigma^2_u$ is ignored in the rescaling as it is only a multiplicative constant of the covariance matrix.}
